...
public class Amplitude {
    ...
    /**
     * Private internal constructor for Amplitude. Please use `getInstance(String name)` or `getInstance()` to get a new
     * instance.
     */
     ...
     *
     * @param instanceName
     *            The key (unique identifier) that matches to the Amplitude instance
     * @return the Amplitude instance that should be used for instrumentation
     */
    ...
    /**
     * Set the API key for this instance of Amplitude. API key is necessary to authorize and route events to the current
     * Amplitude project.
     *
     * @param key
     *            the API key from Amplitude website
     */
    ...
    /**
     * Sends events to a different URL other than Constants.API_URL or Constants.BATCH_API_URL. Used for proxy server.
     *
     * @param url
     *            the server URL for sending event
     */
    ...
    /**
     * Set the Event Upload Mode. If isBatchMode is true, the events will log through the Amplitude HTTP V2 Batch API.
     *
     * @param isBatchMode
     *            if using batch upload or not;
     */
     ...
     *
     * @param logMode
     *            Messages at this level and higher (more urgent) will be logged in the console.
     */
    ...
    /**
     * Sets event upload threshold. The SDK will attempt to batch upload unsent events every eventUploadPeriodMillis
     * milliseconds, or if the unsent event count exceeds the event upload threshold.
     *
     * @param eventUploadThreshold
     ...
     * eventUploadPeriodMillis milliseconds, or if the unsent event count exceeds the * event upload threshold.
     *
     * @param eventUploadPeriodMillis
     ...
     *
     * @param callbacks
     *            AmplitudeCallbacks or null to clean up.
     */
     ...
     *
     * @param proxy
     *            Proxy object, default to Proxy.NO_PROXY for direct connection
     */
     ...
     *
     * @param timeout
     *            the flush events threads timeout millis
     */
     ...
     *            the thread pool for sending events
     */
     ...
     *            the thread pool for retrying events
     */
     ...
     *            The event to be sent
     * @param extra
     *            The extra unstructured data for middleware
     */
     ...
     *            The event to be sent
     * @param callbacks
     *            The callback for the event, this will run in addition to client level callback
     */
    ...
    /**
     * Forces events currently in the event buffer to be sent to Amplitude API endpoint. Only one thread may flush at a
     * time. Next flushes will happen immediately after.
     */
    ...
    /**
     * Check the status of the client, return true if any of the following situation: 1. Events sit in memory waiting
     * for flush are more than flush threshold 2. Events in retry buffer are more than 16,000 3. When
     * setRecordThrottledId(true), the userId or deviceId of the input event was throttled and retry attempt for the
     * userId/deviceId not finished. Can be called before logEvent(event) and add some wait time if return true.
     *
     * @param event
     *            The event to be sent.
    ...
    /**
     * Release resource by: Shutdown threadspool for executing flush events task. All events hold by these threads will
     * trigger callbacks if amplitude client have configured callbacks method. Shutdown client instance stop sending
     * further events.
     */
