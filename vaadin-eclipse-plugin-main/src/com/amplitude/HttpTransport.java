...
class EventsRetryResult {
    ...
    protected EventsRetryResult(boolean shouldRetry, boolean shouldReduceEventCount, int[] eventIndicesToRemove,
...
class HttpTransport {
        ...
        synchronized (bufferLock) {
            for (String userId : idToBuffer.keySet()) {
                for (String deviceId : idToBuffer.get(userId).keySet()) {
                    triggerEventCallbacks(idToBuffer.get(userId).remove(deviceId), 0,
                            "Client shutdown. Events not retry.");
                }
    ...
    private EventsRetryResult retryEventsOnce(String userId, String deviceId, List<Event> events)
        ...
        return new EventsRetryResult(shouldRetry, shouldReduceEventCount, eventIndicesToRemove, response.code,
                response.error);
    }
    ...
    protected boolean shouldRetryForStatus(Status status) {
        return (status == Status.INVALID || status == Status.PAYLOAD_TOO_LARGE || status == Status.RATELIMIT
                || status == Status.TIMEOUT || status == Status.FAILED);
    }
    ...
    public boolean shouldWait(Event event) {
        if (recordThrottledId
                && (throttledUserId.containsKey(event.userId) || throttledDeviceId.containsKey(event.deviceId))) {
            return true;
        }
    ...
    class SendEventsTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                callbackMessage = "Error sending events due to the exception: " + exception + ". Message: "
